...
function canvia_seccio(num_boto) {
    ...
    for (let i = 1; i < num_botons; i++) {
        ...
        if (i == num_boto) {
            boto.style.color = "#338BF0";    // es destaca la secció activa amb el canvi de colors del botó corresponent
            boto.style.backgroundColor = "#D4E4F7";
            seccio.style.display = "flex";    // es fa visible la secció activa
        }
        else {
            boto.style.color = "white";    // colors dels botons de seccions inactives
            boto.style.backgroundColor = "#338BF0";
...
function peticio() {
    const canal = "2897203";    // s'han de substituir els asteriscs pel codi del canal
